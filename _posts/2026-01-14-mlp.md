...
Các mô hình tuyến tính (như Linear Regression hay Softmax Regression) thường gặp khó khăn với dữ liệu phức tạp không thể phân tách tuyến tính. **Multilayer Perceptrons (MLP)** giải quyết vấn đề này bằng cách xếp chồng nhiều lớp nơ-ron lên nhau.
...
Nếu chỉ xếp chồng các lớp tuyến tính ($Wx + b$), toàn bộ mạng vẫn chỉ tương đương với một mô hình tuyến tính đơn lẻ. Để mạng học được các đặc trưng phức tạp, chúng ta cần áp dụng **Hàm kích hoạt (Activation Function)** phi tuyến tính sau mỗi lớp ẩn.
