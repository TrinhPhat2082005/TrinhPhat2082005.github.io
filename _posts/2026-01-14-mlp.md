...
Hàm kích hoạt quyết định một nơ-ron có "phát hỏa" hay không và phá vỡ tính tuyến tính của mô hình. Nếu không có chúng, mạng nơ-ron dù sâu đến đâu cũng chỉ là một hàm hồi quy tuyến tính đơn giản.

### a. ReLU (Rectified Linear Unit)
Đây là hàm phổ biến nhất hiện nay và là sự lựa chọn mặc định khi bắt đầu xây dựng mô hình.

$$
\operatorname{ReLU}(x) = \max(x, 0)
$$

![Hình Minh Họa](/assets/images/relu.png){: .align-center width="100%"}

* **Ưu điểm:**
    * **Tính toán cực nhanh:** Chỉ cần so sánh với 0, không cần tính toán hàm mũ phức tạp.
    * **Giảm thiểu Vanishing Gradient:** Đạo hàm bằng 1 khi $x > 0$, giúp gradient truyền ngược về các lớp đầu tốt hơn.
    * **Tính thưa (Sparsity):** Một số nơ-ron sẽ có đầu ra bằng 0, giúp mạng nhẹ hơn và giảm overfitting.
* **Nhược điểm:**
    * **Dying ReLU:** Nếu đầu vào âm, đạo hàm bằng 0, nơ-ron sẽ "chết" và không bao giờ cập nhật trọng số được nữa.
    * **Không Zero-centered:** Đầu ra luôn dương (từ 0 đến $+\infty$).
* **Khi nào dùng:** Mặc định sử dụng cho tất cả các **Lớp ẩn (Hidden Layers)** trong CNN, MLP.

### b. Sigmoid
Hàm hình chữ S, chuyển đổi đầu vào thành giá trị xác suất trong khoảng $(0, 1)$.

$$
\operatorname{sigmoid}(x) = \frac{1}{1 + \exp(-x)}
$$

![Hình Minh Họa](/assets/images/sigmoid.png){: .align-center width="100%"}

* **Ưu điểm:**
    * **Ý nghĩa xác suất:** Đầu ra luôn nằm trong khoảng $(0, 1)$, rất phù hợp để biểu thị xác suất.
    * **Đạo hàm mượt:** Luôn có đạo hàm tại mọi điểm.
* **Nhược điểm:**
    * **Vanishing Gradient (Nghiêm trọng):** Khi $x$ quá lớn hoặc quá nhỏ, đạo hàm tiến về 0, khiến mạng sâu không thể học được.
    * **Tốn chi phí tính toán:** Do phải tính hàm mũ $e^{-x}$.
    * **Không Zero-centered:** Làm việc tối ưu hóa gradient phức tạp hơn.
* **Khi nào dùng:** Hầu như **KHÔNG** dùng ở lớp ẩn. Chỉ dùng ở **Lớp đầu ra (Output Layer)** cho bài toán **Phân loại nhị phân (Binary Classification)**.

* **Biến thể SWISH** Linh hoạt với tham số $\beta$ có thể tùy chỉnh. Thường dùng trong CV, vì nhanh hội tụ và hoạt động ổn định.

$$X sigmoid(\beta X)$$

### c. Tanh (Hyperbolic Tangent)
Tương tự Sigmoid nhưng đưa giá trị về khoảng $(-1, 1)$.

$$
\tanh(x) = \frac{1 - \exp(-2x)}{1 + \exp(-2x)}
$$

![Hình Minh Họa](/assets/images/tanh.png){: .align-center width="100%"}

* **Ưu điểm:**
    * **Zero-centered:** Dữ liệu đầu ra tập trung quanh 0 (có cả âm và dương), giúp việc tối ưu hóa dễ dàng hơn Sigmoid.
* **Nhược điểm:**
    * **Vẫn bị Vanishing Gradient:** Giống Sigmoid, khi đầu vào quá lớn hoặc quá nhỏ, đạo hàm vẫn tiến về 0 (bão hòa).
* **Khi nào dùng:** Thường dùng trong các kiến trúc mạng hồi quy (RNN, LSTM) hoặc khi chuẩn hóa dữ liệu đầu vào về khoảng $[-1, 1]$. Hiếm khi dùng trong các mạng MLP/CNN hiện đại.
...
[Link code from From Scratch](https://colab.research.google.com/drive/1gY787_CMo1Ib_uVYeOBYKyWp2Vq4_xMs)
...
### **Tổng quan vấn đề**
**Khởi Tạo Tham Số:** Đóng một vai trò rất quan trọng trong mạng nơ-ron học sâu (Deep Neural Network). Nó ảnh hưởng đến tốc độ hội tụ, nếu không cẩn thận thì trong quá trình huấn luyện (training) rất dễ gặp các vấn đề về ổn định số (Numerical Stability). Hai vấn đề thường gặp đó là : Biến mất Gradient (Vanishing Gradients) và Bùng nổ Gradient (Exploding Gradients).

### **Các vấn đề về Gradient**

**Vanishing Gradient**

* **Định Nghĩa:** Gradient trở nên quá nhỏ ~0, khi lan truyền ngược qua các lớp sâu thì tham số không thể cập nhật (không thể học).

* **Nguyên Nhân:** Thường do các hàm kích hoạt (activation function). Ví dụ điển hình là sigmoid.

* Do đạo hàm của hàm sigmoid chỉ chạy từ 0 -> 1, khi nhân vào trong quá trình backward sẽ làm cho Gradient trở nên ->0.

**Exploding Gradient**

* **Định Nghĩa:** Trái ngược với Vanishing , Exploding làm cho gradient trở nên quá lớn, các tham số cập nhật hỗn loạn, phá hủy mô hình.

* **Nguyên Nhân:** Có thể đến từ các nguyên nhân sau : Đạo Hàm của hàm loss quá lớn (Hàm loss sai), các trọng số bị khởi tạo quá lớn,...

**Phá vỡ tính đối xứng (Breaking the symmetry)** 
* **Định Nghĩa:** Nếu khởi tạo tất cả các giá trị trong cùng một lớp giống nhau(các giá trị trong cùng một W bằng nhau). Thì khi cập nhật từng giá trị sẽ thay đổi giống nhau, điều đó có nghĩa là cả lớp như chỉ có 1 nơ-ron. 
* **Giải pháp:**  Khởi tạo random.
### **Khởi tạo tham số**

**Khởi tạo mặc định (Default Initialization):**
Các framework Deep Learning thường có cách khởi tạo mặc định (thường là ngẫu nhiên trong một khoảng nhỏ), nhưng không đảm bảo hoạt động tốt cho mọi mạng sâu.

**Khởi tạo Xavier (Glorot Initialization):**
Đây là phương pháp phổ biến được đề xuất để giữ cho quy mô của gradient ổn định qua các lớp.

* **Mục tiêu:** Giữ cho phương sai (variance) của đầu ra mỗi lớp bằng phương sai của đầu vào lớp đó; đồng thời phương sai của gradient trước và sau khi đi qua một lớp cũng phải bằng nhau.

* **Công thức:**

  * Nếu dùng phân phối chuẩn (Gaussian): Khởi tạo trọng số với trung bình $\mu = 0$ và phương sai $\sigma^2 = \frac{2}{n_{in} + n_{out}}$.

  * Nếu dùng phân phối đều (Uniform): Khởi tạo trong khoảng $U(-a, a)$ với $a = \sqrt{\frac{6}{n_{in} + n_{out}}}$.

  * (Trong đó $n_{in}$ là số lượng đầu vào và $n_{out}$ là số lượng đầu ra của lớp).
